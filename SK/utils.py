...
def readCoords(options,user_data):
    """
    Checks that arguments make sense, and if so returns data read from input file. 
    """
    data = np.array([(entry['RA'], entry['Dec'], float(entry['SN'])) for entry in user_data],
                dtype=[('RA', '<U10'), ('Dec', '<U10'), ('SN', float)])
...
def printCoords(max_loc, w):
    if len(max_loc) == 2:
        ...
        message = '\nMaximum likelihood at coordinates: <br>'
        message += '<p style="color:red">{:.3f}, {:.3f} deg | '.format(max.ra.deg[0], max.dec.deg[0])

        message += max.ra.to_string(u.hour,precision=3)[0] + ', ' + max.dec.to_string(u.degree, alwayssign=True,precision=3)[0] + '</p>'
        return message
